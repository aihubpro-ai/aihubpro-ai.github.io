export async function onRequestPost(context) {
  ...
  try {
    ...
    const ILLEGAL_KEYWORDS = [
      ...
      'suicide', 'kill yourself', 'terrorist', 'isis', 'child porn'
    ];
    ...
    if (paidUntil && Date.now() < parseInt(paidUntil)) {
      ...
      if (!trialStart) {
        ...
        if (elapsed > 5 * 60 * 1000) { // 5 min trial
...
8. Never mention you are an AI unless asked.`;
    ...
    for (const p of aiProviders) {
      ...
      try {
        ...
        // ✅ YE 3 LINES FIX KARI HAIN - Line 199-201 ke aas paas
        if (p.name === 'Gemini') {
        ...
        if (reply) {
          usedProvider = p.name;
          await env.SYSTEM_LOGS?.put(`success_${Date.now()}`, JSON.stringify({
            provider: p.name, 
            ip: ip,
            timestamp: new Date().toISOString()
          }));
        ...
        await env.ERROR_LOGS?.put(`fail_${p.name}_${Date.now()}`, JSON.stringify({
          provider: p.name,
          error: e.message,
          ip: ip,
          timestamp: new Date().toISOString()
    ...
    if (!reply) {
      await env.ALERT_KV?.put(`CRITICAL_DOWN_${Date.now()}`, JSON.stringify({
        time: new Date().toISOString(),
        ip: ip,
        message: 'All AI providers failed'
      }));
      return Response.json({ 
        error: 'All AI services temporarily down. 2 minute baad try karo.' 
